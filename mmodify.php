...
<tr class='b'>
<form action='mprocess.php?id=$id&cblock=$cblock&ccpage=$ccpage&d=$d' method=post>
<td><font size=4>이름</font></td>
<td><b>$UserID</td>
<td><font size=4>메모</font></td>
<td><input type=text size=30 maxlength=100 name=wmemo class='login' value='$message'>
<input type=submit value='수정완료' class='a' style='font-family:Dongle; font-size:20px;'></td>
</form>
...
echo ("
	...
	while ( $i   < $pagesize ) :
		...
		$date = mysql_result($result, $counter, "wdate");
		$message = mysql_result($result,   $counter, "message");
		if ($d == $date){
		echo ("
			<tr class='b'><td style='background-color:#D2E1AA;'>$name</td>
			<td style='background-color:#D2E1AA;'>$date</td><td style='background-color:#D2E1AA;'>$message</td>
			<td style='background-color:#D2E1AA;'><button class='a'>수정중</button>&nbsp;<a href='mdelete.php?wdate=$date&id=$id'><button class='a'>삭제</button></a></td></tr>
		");
		}
		else {
			echo ("
			...
			<td><a href='mmodify.php?wdate=$wdate&id=$id&cpage=$cpage&cblock=$cblock&ccpage=$ccpage'><button class='a'>수정</button></a>&nbsp;<a href='mdelete.php?wdate=$date&id=$id'><button class='a'>삭제</button></a></td></tr>
		");
		}
		$i++;
...
echo("<table class='aa'   border=0 width=700><tr class='b' style='border-color:white;'><td><br></td></tr>
	...
	<a href=pass.php?board=testboard&id=$id&mode=0><button class='a'>수정</button></a>
	<a href=pass.php?board=testboard&id=$id&mode=1&num=$num&code=$code><button class='a'>삭제</button></a>
