...
if(mode==2){
$result = mysql_query("select * from testboard where $field like '%$key%' order by star asc", $con);
}
else{
$result = mysql_query("select * from testboard where $field like '%$key%' order by star desc", $con);
}
...
echo("<hr style='width:500px; height:2px; position:relative; top:30px; right:360px; margin-top:150px;' color=#738B32>
	...
	<button style='background-color:white; position:absolute; top:430px;' class='p'><i class='fa-sharp fa-solid fa-magnifying-glass' style='font-size:18px;'></i></button>
	</td>
	</form>");
	if($mode==2){
		echo("
	<td align=right valign=bottom><a href='testboard.php?mode=1&field=$field&key=$key' style='color:black;'>별점높은순</a>&nbsp;&nbsp;<a href='testboard.php?mode=2&field=$field&key=$key' style='color:#2C9100;'><b>별점낮은순</b></a></td>
	");
	}
	else {
		echo("
	<td align=right valign=bottom><a href='testboard.php?mode=1&field=$field&key=$key' style='color:#2C9100;'><b>별점높은순</b></a>&nbsp;&nbsp;<a href='testboard.php?mode=2&field=$field&key=$key' style='color:black;'>별점낮은순</a></td>
	");
	}
	echo("
	</form>
...
if (!$total){
	...
	while($counter<$pagesize):
		...
		if ($writer == 'admin'){
			...
			if ($to>0):
				echo("
					<tr><td align=center height=150 style='padding:20 0 20 0; border-bottom:0.5px solid lightgray;'>$id<br>(<i class='fa-solid fa-star' style='color:#F9C206;'></i> $star)</td>
					<td align=center height=150 style='padding:20 0 20 0; border-bottom:0.5px solid lightgray;'>$writer</td>
					<td align=left height=150 style='padding:20 0 20 0; border-bottom:0.5px solid lightgray;'><i class='fa-solid fa-circle-exclamation' style='font-size:30px; color:red;'></i>&nbsp<font color=red>&nbsp&nbsp[공지]</font></td>
			...
			if ($to>0):
				echo("
					<tr><td align=center height=150 style='padding:20 0 20 0; border-bottom:0.5px solid lightgray;'>$id<br>(<i class='fa-solid fa-star' style='color:#F9C206;'></i> $star)</td>
					<td align=center height=150 style='padding:20 0 20 0; border-bottom:0.5px solid lightgray;'>$writer</td>
					<td align=left height=150 style='padding:20 0 20 0; border-bottom:0.5px solid lightgray;'><i class='fa-solid fa-circle-exclamation' style='font-size:30px; color:red;'></i><font color=red>&nbsp&nbsp[공지]</td>
...
echo ("<table border=0 width=900 style='margin-bottom:150px;margin-top:20px;'>
	...
	if ($pblock > 0)        // 이전 블록이 존재하면 [이전블록] 버튼을 활성화
		echo ("<a href=search.php?board=testboard&cblock=$pblock&cpage=$pstartpage&field=$field&key=$key><i class='fa-sharp fa-solid fa-chevron-left' style='font-size:20px; position:relative; top:4px; color:black;'></i></a> ");
	...
	while($i < $nstartpage):
	   ...
	   if ($cpage==$i) {
	   echo ("<a href=search.php?board=testboard&cblock=$cblock&cpage=$i&field=$field&key=$key><button style='background-color:#62A834; border-radius:50px; color:white; padding:5px 10px 5px 10px;' nowrap>$i</button></a>&nbsp;");
	   $i = $i + 1;
	   }
	   else{
		   echo ("<a href=search.php?board=testboard&cblock=$cblock&cpage=$i&field=$field&key=$key><button class='a' id='b' nowrap>$i</button></a>&nbsp;");
	   $i = $i + 1;
	   }
	endwhile;
	...
	if ($nstartpage <= $totalpage)   
		echo ("<a href=search.php?board=testboard&cblock=$nblock&cpage=$nstartpage&field=$field&key=$key><i class='fa-sharp fa-solid fa-chevron-right' style='font-size:20px; position:relative; top:4px; color:black;'></i></a> ");
