...
switch($class) {
   case 0:        // 초기화면에 출력할 인기 상품 목록
	   ...
	   switch($mode) {
			...
			case 2:        // 초기화면에 출력할 인기 상품 목록
				$result = mysql_query("select * from product where name like '%$key%' order by price2 asc", $con);
	   
				break;
			case 3:        // 초기화면에 출력할 인기 상품 목록
				$result = mysql_query("select * from product where name like '%$key%' order by price2 desc", $con);
   ...
   default:       // 카테고리별 메뉴 화면에 출력할 상품 목록
       $result = mysql_query("select * from product where class=$class and name like '%$key%'", $con);
	   switch($mode) {
			case 0:        // 초기화면에 출력할 인기 상품 목록
				$result = mysql_query("select * from product where class=$class and name like '%$key%' order by hit desc", $con);
			...
			case 1:        // 초기화면에 출력할 인기 상품 목록
				...
				break;
			case 2:        // 초기화면에 출력할 인기 상품 목록
				$result = mysql_query("select * from product where class=$class and name like '%$key%' order by price2 desc", $con);
	   
				break;
			case 3:        // 초기화면에 출력할 인기 상품 목록
				$result = mysql_query("select * from product where class=$class and name like '%$key%' order by price2 asc", $con);
...
echo ("<center><table border=0 width=1200 style='margin-left:200px; margin-top:300px; border-bottom:2px solid #97B742; '><tr>
		<td align=right>
		");
if ($mode == 2){
	echo("
<a href='p-search.php?class=$class&mode=2&key=$key' style='font-size:20px; color:#2C9100;'>낮은가격</a>");
}
else {
echo("
<a href='p-search.php?class=$class&mode=2&key=$key' style='color:black;'>낮은가격</a>");
}
if ($mode == 3){
	echo("
<a href='p-search.php?class=$class&mode=3&key=$key' style='font-size:20px; color:#2C9100;'>높은가격</a>");
}
else {
echo("
<a href='p-search.php?class=$class&mode=3&key=$key' style='color:black;'>높은가격</a>");
}
if ($mode == 4){
	echo("
<a href='p-search.php?class=$class&mode=4&key=$key' style='font-size:20px; color:#2C9100;'>리뷰순</a>");
}
else {
echo("
<a href='p-search.php?class=$class&mode=4&key=$key' style='color:black;'>리뷰순</a>");
}
if ($mode == 1){
	echo("
<a href='p-search.php?class=$class&mode=1&key=$key' style='font-size:20px; color:#2C9100;'>조회순</a></td></tr>");
}
else {
echo("
<a href='p-search.php?class=$class&mode=1&key=$key' style='color:black;'>조회순</a></td></tr>");
}
echo ("<center><table border=0 width=1000 style='margin-left:200px;'><tr>");

if (!$total){
	echo ("<td align=center height=250><font color=red>아직 등록된 상품이 없습니다</td>");
} else {
...
echo ("<table border=0 width=700 style='margin-bottom:200px;'>
	...
	if ($pblock > 0)        // 이전 블록이 존재하면 [이전블록] 버튼을 활성화
		echo ("<a href=p-search.php?board=testboard&cblock=$pblock&cpage=$pstartpage&class=$class&mode=$mode&key=$key><i class='fa-sharp fa-solid fa-chevron-left' style='font-size:20px; position:relative; top:4px; color:black;'></i></a> ");
	...
	while($i < $nstartpage):
	   ...
	   if ($cpage==$i) {
	   echo ("<a href=p-search.php?board=testboard&cblock=$cblock&cpage=$i&class=$class&mode=$mode&key=$key><button style='background-color:#62A834; border-radius:100px; color:white;' nowrap>$i</button></a>&nbsp;");
	   $i = $i + 1;
	   }
	   else{
		   echo ("<a href=p-search.php?board=testboard&cblock=$cblock&cpage=$i&class=$class&mode=$mode&key=$key><button class='a' nowrap>$i</button></a>&nbsp;");
	   $i = $i + 1;
	   }
	endwhile;
	...
	if ($nstartpage <= $totalpage)   
		echo ("<a href=p-search.php?board=testboard&cblock=$nblock&cpage=$nstartpage&class=$class&mode=$mode&key=$key><i class='fa-sharp fa-solid fa-chevron-right' style='font-size:20px; position:relative; top:4px; color:black;'></i></a> ");
